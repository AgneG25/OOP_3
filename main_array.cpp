...
struct stud {
    ...
    string firstName;
    string secondName;
    auto *homework = new double [arr_size];
    double exam;
    int arr_size = 0;
    int idx = 1;
    ...
    void wide_array() {
        arr_size *= 2;
        auto *new_arr = new double[arr_size];
        for (int i = 0; i < idx; i++)
            new_arr[i] = homework[i];
        delete [] homework;
        homework = new_arr;
    }
    void Random() {
...
void Read (int N, stud Students[], int &longestName, int &longestSurname) {
    ...
    for (int i = 0; i < N; i++){
        ...
        if(a == 1){
            ...
            cin >> mark;
            if(mark <= 10 && mark >=1){
                if(Students[i].idx + 1 == Students[i].arr_size) 
                    Students[i].wide_array();
                Students[i].homework = mark;
                Students[i].idx++;
            }
            else {
